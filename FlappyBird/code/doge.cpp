...
{
    string shiba_path = "res/image/shiba1.png";
    double scale = 0.8;
    if (isDark)
    {
        shiba_path = "res/image/Up/mordecai0.png";
        scale = 1.8;
    }
    ...
    if (isNULL() || saved_path != shiba_path)   
    {
        ...
        if ( Load(shiba_path.c_str() , scale) )
        {
            return true;
        }
        else
...
void doge::update(short int pipeWidth, short int pipeHeight)
{
    if (!die)
    {
        ...
        else if (posDoge.x > posPipe[ahead].x + pipeWidth/2 )
        {
